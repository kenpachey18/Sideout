(function () {
  ...
  function renderSummary() {
    ...
    $("#summary-rate").textContent = `${money(state.rate)} / hour`; $("#summary-total").textContent = money(total); if ($("#hero-rate")) $("#hero-rate").textContent = `${money(state.rate)} / hour`;
    $("#continue-booking").disabled = !state.selected.length;
    $("#mobile-summary-time").textContent = start === null ? "Choose a time" : `Court ${state.bookingCourt} · ${hourLabel(start)} – ${hourLabel(end)}`;
    $("#mobile-summary-total").textContent = money(total);
    $("#mobile-continue").disabled = !state.selected.length;
    document.body.classList.toggle("has-selection", Boolean(state.selected.length));
  }
  ...
  $("#date-picker").addEventListener("change",event=>event.target.value&&changeDate(event.target.value)); $("#continue-booking").addEventListener("click",openCheckout); $("#mobile-continue").addEventListener("click",openCheckout); $("#close-modal").addEventListener("click",closeCheckout); $("#booking-modal").addEventListener("mousedown",event=>{if(event.target.id==="booking-modal")closeCheckout();});
